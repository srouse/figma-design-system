...
export default function customSatellite() {
  const [tokenGroup] = useSyncedState(
    'tokenGroup',
    defaultTokenGroup
  );

  const [customInitialized, setCustomInitialized] = useSyncedState(
    'customInitialized',
    false
  );
  ...
  const customList = renderList(tokenGroup);
...
function renderList(
  ...
  const sizes = tokens.map(
    (tokenInfo, index) => {
      const token = tokenInfo[1] as DSysCustomToken;
