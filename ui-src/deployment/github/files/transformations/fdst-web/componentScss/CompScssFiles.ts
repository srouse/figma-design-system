...
class CompScssFiles {
  ...
  async upload(
    ...
    loopDesignSystemTokens(
      ...
      (token: DSysToken) => {
        if (token.$type === DTTokenType.component) {
          ...
          compScssFile.path = `transformations/fdst-web/components/${iconName}.scss`;
    ...
    console.log('compScssFiles', compScssFiles);
    updateFeedback(`uploading component scss files: transformations/fdst-web/components/...`);
